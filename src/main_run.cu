...
/*
    ...
    const char* ConvoOutputPath = argv[7];
...
// for proccess 1 Colortransformation
    dim3 blockDim(16, 16);
...
// for proccess 2 mask generating

    // Launching histogram kernel
    unsigned int* d_histogramGrayscale, *d_histogramYUV;
    cudaMalloc((void**)&d_histogramGrayscale, NUM_BINS * sizeof(unsigned int));
    cudaMemset(d_histogramGrayscale, 0, NUM_BINS * sizeof(unsigned int));
	cudaMalloc((void**)&d_histogramYUV, NUM_BINS * sizeof(unsigned int));
    cudaMemset(d_histogramYUV, 0, NUM_BINS * sizeof(unsigned int));
    ...
    computeHistogram<<<numBlocks, threadsPerBlock, NUM_BINS * sizeof(unsigned int)>>>(d_grayscaleImage, d_histogramGrayscale, width * height);
    ...
    computeHistogram<<<numBlocks, threadsPerBlock, NUM_BINS * sizeof(unsigned int)>>>(d_UComponentImage, d_histogramYUV, width * height);
...
cudaMemcpy(histogram, d_histogramGrayscale, NUM_BINS * sizeof(unsigned int), cudaMemcpyDeviceToHost);
